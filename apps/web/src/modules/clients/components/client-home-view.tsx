...
export function ClientHomeView({
  ...
  return (
    <>
      {/* ========================================================================= */}
      {/* 1. MOBILE COMPACT DASHBOARD (< 768px)                                    */}
      {/* ========================================================================= */}
      <div className="md:hidden space-y-3.5">
        ...
        {client?.project_manager && (
          <div className="p-4 rounded-xl border border-border/80 bg-card space-y-3.5 shadow-xs">
            ...
            <div className="flex items-center gap-3.5">
              <div className="relative shrink-0">
                <OrbitAvatar
                  ...
                  size="lg"
                  className="h-14 w-14 ring-1 ring-border/80"
                />
            ...
            <div className="pt-2 border-t border-border/40 grid grid-cols-2 gap-2">
              <a href={`mailto:${client.project_manager.email}`} className="w-full">
                <Button
                  variant="outline"
                  size="sm"
                  className="w-full h-8.5 text-xs font-semibold gap-1.5 border-border/70 hover:bg-secondary cursor-pointer"
                >
              ...
              <Link href="/client/requests" className="w-full">
                <Button
                  size="sm"
                  className="w-full h-8.5 text-xs font-semibold gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 cursor-pointer shadow-xs"
                >
        ...
        {/* CARD 2: NEXT PAYMENT */}
        <div className="p-4 rounded-xl border border-border/80 bg-card space-y-2.5 shadow-xs">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`h-2 w-2 rounded-full ${
                upcomingPayment?.isUnderVerification
                  ? "bg-amber-400 animate-pulse"
                  : upcomingPayment?.latestRejectionReason
                  ? "bg-destructive"
          ...
          {upcomingPayment ? (
            <div className="space-y-1">
              <span className="text-2xl sm:text-3xl font-extrabold font-mono text-foreground block tracking-tight">
            ...
            <div className="space-y-0.5">
              <span className="text-2xl font-bold font-mono text-foreground block">—</span>
              <p className="text-xs text-muted-foreground">No upcoming payment</p>
            </div>
          )}

          <div className="pt-2 border-t border-border/40">
            <Link href="/client/payments" className="w-full block">
              {upcomingPayment?.isUnderVerification ? (
                <Button
                  size="sm"
                  disabled
                  className="w-full h-8.5 text-xs font-semibold bg-secondary/60 text-muted-foreground border border-border/80 cursor-not-allowed opacity-50 select-none pointer-events-none"
                >
                ...
                <Button
                  size="sm"
                  className="w-full h-8.5 text-xs font-semibold gap-1.5 bg-primary text-primary-foreground hover:bg-primary/90 cursor-pointer"
                >
                  <span>Pay Again</span>
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full h-8 text-xs font-semibold justify-between px-2 text-foreground/80 hover:text-foreground cursor-pointer"
                >
        ...
        <div className="p-4 rounded-xl border border-border/80 bg-card space-y-2.5 shadow-xs">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`h-2 w-2 rounded-full ${
                pendingReviewDeliverables.length > 0 ? "bg-amber-400 animate-pulse" : "bg-emerald-400"
              }`}
            />
          </div>
          ...
          <div className="space-y-0.5">
            <span className="text-base font-bold text-foreground block">
              ...
              {pendingReviewDeliverables.length === 1 ? "Deliverable" : "Deliverables"}
            </span>
            <p className="text-xs text-muted-foreground">
              {pendingReviewDeliverables.length > 0
                ? `${pendingReviewDeliverables.length} ${
                    pendingReviewDeliverables.length === 1 ? "item needs" : "items need"
                  } your attention.`
          ...
          <div className="pt-2 border-t border-border/40">
            ...
            >
              <Button
                ...
                className="w-full h-8 text-xs font-semibold justify-between px-2 text-foreground/80 hover:text-foreground cursor-pointer"
              >
                <span>{pendingReviewDeliverables.length > 0 ? "Review Deliverable" : "View"}</span>
            ...
            <div className="space-y-1.5">
              <div className="flex items-baseline justify-between">
                ...
                <span className="text-xs font-bold font-mono text-foreground">
                  {primaryProject.progress}% complete
                </span>
              </div>
              ...
              <ProjectProgressBar
                ...
                completedMilestoneCount={primaryProject.completed_milestone_count}
              />

              <p className="text-[11px] font-mono text-muted-foreground pt-0.5">
                {primaryProject.completed_milestone_count} / {primaryProject.milestone_count} milestones
              </p>
            </div>

            <div className="pt-2 border-t border-border/40">
              <Link href={`/client/projects/${primaryProject.id}`} className="w-full block">
                <Button
                  variant="ghost"
                  size="sm"
                  className="w-full h-8 text-xs font-semibold justify-between px-2 text-foreground/80 hover:text-foreground cursor-pointer"
        ...
        <div className="p-4 rounded-xl border border-border/80 bg-card space-y-2.5 shadow-xs">
          <div className="flex items-center justify-between">
            ...
            <div
              className={`h-2 w-2 rounded-full ${
                activeRequests.length > 0 ? "bg-sky-400" : "bg-emerald-400"
              }`}
            />
          </div>
          ...
          <div className="space-y-0.5">
            <span className="text-base font-bold text-foreground block">
              {activeRequests.length} Open {activeRequests.length === 1 ? "Request" : "Requests"}
            </span>
            <p className="text-xs text-muted-foreground truncate">
              {activeRequests.length > 0
                ? `${activeRequests.length} in progress with team.`
                : "No open inquiries."}
            </p>
          </div>

          <div className="pt-2 border-t border-border/40">
            <Link href="/client/requests" className="w-full block">
              <Button
                variant="ghost"
                size="sm"
                className="w-full h-8 text-xs font-semibold justify-between px-2 text-foreground/80 hover:text-foreground cursor-pointer"
              >
                <span>{activeRequests.length > 0 ? "View Requests" : "New Request"}</span>
                <ArrowRight className="h-3 w-3" />
              </Button>
            </Link>
          </div>
        ...
        <div className="p-4 rounded-xl border border-border/80 bg-card space-y-2.5 shadow-xs">
          ...
          {upcomingMeeting ? (
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground font-mono">
                {formatDateShort(upcomingMeeting.starts_at)} · {formatTime(upcomingMeeting.starts_at)}
              </p>
            </div>
          ) : (
            <div className="space-y-0.5">
          ...
          <div className="pt-2 border-t border-border/40">
            <Link href="/client/meetings" className="w-full block">
              <Button
                variant="ghost"
                size="sm"
                className="w-full h-8 text-xs font-semibold justify-between px-2 text-foreground/80 hover:text-foreground cursor-pointer"
              >
                <span>{upcomingMeeting ? "View Meeting" : "Schedule"}</span>
                <ArrowRight className="h-3 w-3" />
              </Button>
            </Link>
          </div>
          ...
          <div className="divide-y divide-border/40">
            {recentActivities.length === 0 ? (
              ...
              recentActivities.slice(0, 3).map((act) => (
                <div key={act.id} className="py-2 flex items-center justify-between gap-2 text-xs">
      ...
      {/* ========================================================================= */}
      {/* 2. APPROVED DESKTOP / LAPTOP DASHBOARD (>= 768px)                         */}
      {/* ========================================================================= */}
      <div className="hidden md:block space-y-8 max-w-7xl">
                ...
                <div className="space-y-1">
                  ...
                  <div className="pt-0.5">
                    {upcomingPayment.isUnderVerification ? (
                      ...
                      <span className="text-[11px] font-mono text-destructive font-semibold block truncate" title={upcomingPayment.latestRejectionReason}>
                        ✕ Verification failed
                      </span>
                    ) : upcomingPayment.isOverdue ? (
                      <span className="text-[11px] font-mono text-destructive font-semibold block">
